...
<script lang="ts">
    ...
    onMount(async () => {
        ...
        function getModel(
            ...
            if (type === "json") {
                const model =
                    monaco.editor.getModel(jsonModelUri) ||
                ...
                const model =
                    monaco.editor.getModel(tsModelUri) ||
                    monaco.editor.createModel(
                        $valueStore,
                        "typescript",
                        tsModelUri,
    ...
    const SHORTCUTS: ShortcutList = [[runCodeShortcut, runCode]];
</script>
...
<div class="editor" use:shortcuts={SHORTCUTS}>
    <PanelHeader>
        <Select
            ...
            items={LANGUAGE_KEYS.map((value) => ({
                value,
                name: LANGUAGE_NAMES[value],
            }))}
            toggledClass="editor__toggled-select"
            slot="left"
        ...
        >
            <div class="editor__select">
                ...
                <svg
                    ...
                    class="editor__select-icon"
                    viewBox="0 0 23 23"
                    ><path
                        fill="currentColor"
                        d="M3.96 6.648c-.434 0-1.033.717-.499 1.439l7.663 7.888c.33.371.81.396 1.15.025l6.934-8.103c.275-.318.149-1.265-.575-1.28z"
                    /></svg
                >
            </div>
        </Select>
        <div slot="right">
            {#if currentLanguage === "ts"}
                <button
                    ...
                    on:click={runCode}
                    bind:this={runButton}
                    title={$l10n("runTitle").replace(
                        "%s",
                        runCodeShortcut.toString(),
                    )}
                >
                    {#if isRunning}
                        <svg
                        ...
                        >
                            <path
                                ...
                                stroke="currentColor"
                                d="M13 8a5 5 0 0 1-5 5M3 8a5 5 0 0 1 5-5"
                            ></path>
                        </svg>
                    {:else}
                        <svg
